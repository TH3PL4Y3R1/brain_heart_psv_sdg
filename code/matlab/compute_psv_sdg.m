...
% COMPUTE_PSV_SDG
% End-to-end helper that starts from a FieldTrip raw structure (data_eeg)
% and indices of detected heartbeats (pks_indx). It computes CSI/CVI from
% inter-beat intervals, obtains time-resolved EEG power via FieldTrip,
% integrates power within canonical bands, and runs the SDG model per band.
%
% Inputs
% - data_eeg : FieldTrip raw structure with at least .trial{1} (chan x time)
%              and .time{1} (1 x time, in seconds)
% - pks_indx : indices into data_eeg.time{1} for heartbeat peaks (e.g., R-peaks)
%
% Output
% - struct_sdg : a struct gathering time vectors, CSI/CVI, band power, and
%                directional coupling estimates for each EEG band.
%
% Note: This commented version preserves the original function structure
% and logic; only explanatory comments were added.


%% 1) Build inter-beat intervals (IBI) from detected heartbeats
% Convert peak indices into times (seconds) on the FieldTrip time axis
...
%% Poincare
% Window for time-varying Poincaré indices (seconds)
wind = 15;
[CSI_out, CVI_out, t_out] = compute_CSI_CVI(IBI, t_IBI, wind);

%% frequency analysis
% 2) Time-frequency analysis of EEG (FieldTrip)
% Configure multi-taper convolution with 2 s windows and 50% overlap
cfg              = [];
cfg.output       = 'pow';
...
cfg.pad          = 'nextpow2';
cfg.foi          = 0:0.5:45;                         
cfg.t_ftimwin    = ones(length(cfg.foi),1).*2;  % 2 s windows for all freqs
cfg.toi          = '50%';  % 50% overlap                
freq = ft_freqanalysis(cfg, data_eeg);

%% 3) Align CSI/CVI with the frequency analysis time support
time2 = freq.time;
...
%% 4) Integrate power within canonical frequency bands
...
% Integrate power spectrogram over frequency using trapezoidal rule
...
%% 5) Define plotting/interpretation time vectors for couplings
win_RR = 15; % seconds
time = time3;
...
%% 6) Run SDG model per band (1 Hz time base)
Fs = 1; % resampling for model alignment
